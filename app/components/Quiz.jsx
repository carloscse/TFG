...
import {onAnswerTrue, onAnswerFalse, finishApp, next, restart} from './../reducers/actions';
...
export default class Quiz extends React.Component {
  ...
  onAnswerTrue(){
    this.props.dispatch(onAnswerTrue());
    // Calculate score
    /* let nChoices = this.props.quiz.choices.length;
    let correctAnswers = 0;
    for(let i = 0; i < nChoices; i++){
      let choice = this.props.quiz.choices[this.props.game.index];
      if(this.props.game.index === 0){
        // Answered choice
    ...
    this.props.game.nAnswers++;
    this.props.game.score = this.props.game.score + Math.max(correctAnswers * 10);
  ...
  onAnswerFalse(){
    this.props.dispatch(onAnswerFalse());
      // Calculate score
    /* let nChoices = this.props.quiz.choices.length;
    let correctAnswers = 0;
    for(let i = 0; i < nChoices; i++){
  ...
  onNextQuestion(){
    ...
    } else if(this.props.game.nAnswers === this.props.quiz.choices.length){
  ...
  onRestartQuiz(){
    ...
    /* this.props.game.score = 0;
      this.props.game.index = 0;
      this.props.game.nAnswers = 0;
      this.setState({selected_choices_titles:[], answered:false});*/
  }
      ...
      appContent = (
        <div className="quiz">
          ...
          <div className="quizButtonsWrapper">
            <button className="restart" onClick={this.onRestartQuiz.bind(this)}>{this.props.I18n.getTrans("i.restart")}</button>
